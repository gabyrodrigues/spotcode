class Api::V1::DashboardController < ApplicationController
    ...
    def load_recommendations
      ...
      if heard_categories.present?
        @recommended_albums = Album.joins(:category, :songs).where(category: heard_categories).order("songs.played_count").select("distinct albums.*, songs.played_count").limit(4)
      else
